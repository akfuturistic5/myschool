...
async function main() {
  ...
  try {
    const sqlPath = path.join(__dirname, '..', 'migrations', '006_global_settings.sql');
    if (!fs.existsSync(sqlPath)) {
      console.warn('Skip (missing): 006_global_settings.sql');
      return;
    }
    const sql = fs.readFileSync(sqlPath, 'utf8');
    console.log('Applying 006_global_settings.sql...');
