...
const listSectionStudents = async (req, res) => {
  try {
    const classSectionId = req.query.class_section_id ?? req.query.classSectionId;
    const academicYearId = req.query.academic_year_id ?? req.query.academicYearId;
    const result = await homeworkService.listSectionStudentsForPicker(
      classSectionId,
      academicYearId,
      req.user
    );
    ...
    return success(res, 200, 'Section students fetched successfully', result.students, {
      count: result.students.length,
    });
  } catch (err) {
    console.error('listSectionStudents:', err);
    return errorResponse(res, 500, 'Failed to fetch section students');
  }
};
